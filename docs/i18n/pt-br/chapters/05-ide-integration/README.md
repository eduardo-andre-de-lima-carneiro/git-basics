# Capítulo 5: Integração com IDEs e editores

A maior parte do trabalho diário com Git acontece dentro de um editor de código ou IDE. Os comandos dos capítulos anteriores continuam rodando por baixo; o editor acrescenta uma camada visual para preparar mudanças, ver diferenças, consultar o histórico, trocar de branch e resolver conflitos. Este capítulo explica primeiro as ideias compartilhadas e depois mostra como elas aparecem no Visual Studio Code, no Visual Studio e nas IDEs da JetBrains, seguido de um panorama de outros editores e de uma nota sobre fluxos assistidos por IA.

- [Fundamentos da integração com IDEs](01-ide-integration-fundamentals.md)
- [Visual Studio Code](02-vscode.md)
- [Visual Studio](03-visual-studio.md)
- [IDEs da JetBrains](04-jetbrains.md)
- [Outros editores](05-other-editors.md)
- [Fluxos assistidos por IA](06-ai-assisted-workflows.md)
- [Configuração de editor e ferramenta de merge](07-editor-and-mergetool-config.md)
- [Exercícios práticos](08-exercises.md)

## Referências

Cada lição lista a documentação oficial em que se baseia. As fontes principais deste capítulo são:

- [Pro Git (2ª edição)](https://git-scm.com/book/en/v2) e o [manual de referência do Git](https://git-scm.com/docs)
- [Visual Studio Code — documentação de controle de versão](https://code.visualstudio.com/docs/sourcecontrol/overview)
- [Microsoft Learn — Git no Visual Studio](https://learn.microsoft.com/pt-br/visualstudio/version-control/git-with-visual-studio)
- [JetBrains — integração com Git no IntelliJ IDEA](https://www.jetbrains.com/help/idea/using-git-integration.html)
